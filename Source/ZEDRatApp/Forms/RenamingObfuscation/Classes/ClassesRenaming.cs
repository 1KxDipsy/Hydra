	...
	public class ClassesRenaming : IRenaming
	{
		...
		public ModuleDefMD Rename(ModuleDefMD module)
		{
			foreach (TypeDef type in module.GetTypes())
			{
				...
				{
					if (ClassesRenaming._names.TryGetValue(type.Name, out var value))
					{
						type.Name = value;
						continue;
					}
					string text = Utils.GenerateRandomString();
					ClassesRenaming._names.Add(type.Name, text);
					type.Name = text;
				}
			}
			...
			foreach (TypeDef type in module.GetTypes())
			{
				foreach (PropertyDef property in type.Properties)
				{
					...
					for (int i = 0; i < instructions.Count; i++)
					{
						...
						foreach (KeyValuePair<string, string> name2 in ClassesRenaming._names)
						{
