	...
	public class Button : CustomButtonBase
	{
			...
			checked
			{
				...
				while (true)
				{
					if (num3 <= num2)
					{
						...
						double progress = base.AnimationManager.GetProgress(num3);
						Point source = base.AnimationManager.GetSource(num3);
						using (Brush brush = new SolidBrush(Color.FromArgb((int)Math.Round(101.0 - progress * 100.0), base.DefaultPressedColor)))
						{
							int num4 = (int)Math.Round(progress * (double)num * 2.0);
							graphics.FillEllipse(brush, new Rectangle((int)Math.Round((double)source.X - (double)num4 / 2.0), (int)Math.Round((double)source.Y - (double)num4 / 2.0), num4, num4));
						}
						num3++;
						continue;
					}
		...
		private void method_20(Graphics graphics_0)
		{
			...
			if (base.DefaultBorderRadius > 0)
			{
				...
				{
					Class6.smethod_20(graphics_0, flag ? new SolidBrush(base.DefaultFocusedColor) : new SolidBrush(color4), base.ClientRectangle, base.DefaultBorderRadius, (base.DefaultBorderThickness > 0) ? base.DefaultBorderThickness : (flag ? 1 : 0), base.DefaultBorderStyle);
				}
			}
			else
			{
				...
				Class6.smethod_22(graphics_0, flag ? new SolidBrush(base.DefaultFocusedColor) : new SolidBrush(color4), base.ClientRectangle, (base.DefaultBorderThickness > 0) ? base.DefaultBorderThickness : (flag ? 1 : 0), base.DefaultBorderStyle);
			}
			Class6.smethod_13(graphics_0, color3, base.ClientRectangle, base.DefaultCustomBorderThickness, base.DefaultBorderRadius);
			...
			graphics_0.DrawString(s, font, new SolidBrush(color2), rectangle, new StringFormat
			{
