...
namespace ns2
{
	...
	public class ImageRadioButton : UIDefaultControl
	{
		...
		{
			if (!base.IsHandleCreated || !this.bool_4)
			{
				return;
			}
			foreach (Control control in base.Parent.Controls)
			{
				if (control != this && control is ImageRadioButton && control != this)
				{
					ImageRadioButton imageRadioButton = (ImageRadioButton)control;
					if (imageRadioButton.DefaultChecked)
					{
						imageRadioButton.DefaultChecked = false;
					}
				}
			}
		}
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			if (!base.Enabled)
			{
				...
				this.method_11(Graphics.FromImage(image));
				ControlPaint.DrawImageDisabled(e.Graphics, image, 0, 0, Color.White);
			}
			else
			{
				this.method_11(e.Graphics);
			}
			base.OnPaint(e);
		}
	}
